...
public class BoxOctree {
    ...
    private int roundAwayFromZero(double value) {
        return value >= (double)0.0F ? (int)Math.ceil(value) : (int)Math.floor(value);
    }

    private void subdivide() {
        if (!this.childrenOctants.isEmpty()) {
            throw new UnsupportedOperationException("ott - Tried to subdivide when there are already children octants.");
        } else {
