...
public class LargeDripstoneFeature extends Feature<LargeDripstoneConfig> {
    ...
    public boolean place(FeaturePlaceContext<LargeDripstoneConfig> context) {
        ...
        if (DripstoneUtils.isEmptyOrWater(level, origin)) {
            ...
            if (column.isPresent()) {
                ...
                if (var8 instanceof Column.Range range) {
                    if (range.height() < 4) {
                        return false;
                    }

                    int unclampedRadius = (int) ((float) range.height() * config.maxColumnRadiusToCaveHeightRatio());
                    ...
                    LargeDripstone ceilingDripstone = makeDripstone(config.stateProvider(), random, origin.atY(range.ceiling() - 1), false, radius, config.stalactiteBluntness(), config.heightScale());
                    LargeDripstone floorDripstone = makeDripstone(config.stateProvider(), random, origin.atY(range.floor() + 1), true, radius, config.stalagmiteBluntness(), config.heightScale());
    ...
    static final class WindOffsetter {
        ...
        WindOffsetter(int y, RandomSource random, FloatProvider $$2) {
            ...
            this.windSpeed = new Vec3(MathUtils.cos($$4) * $$3, 0.0F, MathUtils.sin($$4) * $$3);
        }
