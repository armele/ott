    ...
    public void tick() {
        ...
        if (!this.animal.isInWater() && this.isReachedTarget()) {
            if (this.animal.getLayEggCounter() < 1) {
                ...
                BlockState eggState = null;
                if (this.animal.getEggBlock() != null) {
                    eggState = this.animal.getEggBlock().defaultBlockState();
                    if (eggState.hasProperty(TurtleEggBlock.EGGS)) {
                        eggState = eggState.setValue(TurtleEggBlock.EGGS, eggCount);
                    }
                }
