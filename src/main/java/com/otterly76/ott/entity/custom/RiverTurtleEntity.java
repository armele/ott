...
public class RiverTurtleEntity extends Animal implements GeoEntity {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    private static final RawAnimation SWIM_ANIMATION = RawAnimation.begin().thenLoop("Swim");
    private static final RawAnimation WALK_ANIMATION = RawAnimation.begin().thenLoop("Land");
    private static final RawAnimation IDLE_ANIMATION = RawAnimation.begin().thenLoop("Idle");

    public RiverTurtleEntity(EntityType<? extends Animal> type, Level level) {
        super(type, level);
    ...
    public AgeableMob getBreedOffspring(@NotNull ServerLevel level, @NotNull AgeableMob other) {
        return ModEntities.RIVER_TURTLE.get().create(level);
    }
