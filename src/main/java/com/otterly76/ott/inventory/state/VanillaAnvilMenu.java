...
public class VanillaAnvilMenu extends BuiltInAnvilMenu {
    ...
    public void createResult() {
        ...
        ((AnvilMenuAccessor) this).ott$getCost().set(1);
        int i = 0;
        long l = 0L;
        int j = 0;
        if (!itemStack.isEmpty() && EnchantmentHelper.canStoreEnchantments(itemStack)) {
            ...
            if (!itemStack3.isEmpty()) {
                ...
                if (itemStack2.isDamageableItem() && itemStack2.getItem().isValidRepairItem(itemStack, itemStack3)) {
                    ...
                    ((AnvilMenuAccessor) this).ott$setRepairItemCountCost(m);
                } else {
                    ...
                    for(Object2IntMap.Entry<Holder<Enchantment>> entry : itemEnchantments.entrySet()) {
                        ...
                        ContainerMenuHelper.EnchantmentCompatibilityResult compatibilityResult = ContainerMenuHelper.checkEnchantmentCompatibility(holder, itemStack, mutable.keySet(), this.player.getAbilities().instabuild);
                        i += compatibilityResult.costAddition();
            ...
            String itemName = ((AnvilMenuAccessor) this).ott$getItemName();
            ...
            ((AnvilMenuAccessor) this).ott$getCost().set(t);
            if (i <= 0) {
                itemStack2 = ItemStack.EMPTY;
            }

            if (j == i && j > 0 && ((AnvilMenuAccessor) this).ott$getCost().get() >= 40) {
                ((AnvilMenuAccessor) this).ott$getCost().set(39);
            }

            if (((AnvilMenuAccessor) this).ott$getCost().get() >= 40 && !this.player.getAbilities().instabuild) {
                itemStack2 = ItemStack.EMPTY;
            }
