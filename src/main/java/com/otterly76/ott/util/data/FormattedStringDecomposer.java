...
public class FormattedStringDecomposer {
    public static int stringWidth(Font font, @Nullable String content, int skip) {
        if (content == null) {
            ...
            iterateFormatted(content, Style.EMPTY, (index, style, codePoint) -> {
                if (index >= skip) {
    ...
    public static void iterateFormatted(String text, Style defaultStyle, FormattedCharSink sink) {
        ...
        for(int position = 0; position < textLength; ++position) {
            ...
            if (character == 167) {
                if (position + 1 >= textLength) {
                    feedChar(text, defaultStyle, sink, position, character, textLength);
                    return;
                }
    ...
    public static void iterateFormattedBackwards(String text, Style defaultStyle, FormattedCharSink sink) {
