...
public class ComponentDecomposer {
    ...
    private static String applyLegacyFormatting(String string, Style style) {
        return toLegacyFormatting(style).stream().map(ChatFormatting::toString).reduce(String::concat).map((formattings) -> formattings.concat(string).concat(ChatFormatting.RESET.toString())).orElse(string);
    }
    ...
    public static Component toFormattedComponent(@Nullable String value) {
        return toComponentEntries(value).stream().map((entry) -> Component.literal(entry.getValue()).withStyle(entry.getStyle())).reduce(MutableComponent::append).orElse(Component.empty());
    }
    ...
    private static Deque<ComponentEntry> toComponentEntries(@Nullable String value) {
        ...
        StringDecomposer.iterateFormatted(value, EMPTY, (index, style, codePoint) -> {
    ...
    static {
        EMPTY = Style.EMPTY.withColor(ChatFormatting.WHITE).withBold(false).withItalic(false).withUnderlined(false).withStrikethrough(false).withObfuscated(false);
    }
