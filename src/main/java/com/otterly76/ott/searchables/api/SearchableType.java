...
import com.otterly76.ott.searchables.api.autcomplete.CompletionSuggestion;
import com.otterly76.ott.searchables.api.context.ContextVisitor;
import com.otterly76.ott.searchables.api.context.SearchContext;
...
public final class SearchableType<T> {
    ...
    public Map<String, SearchableComponent<T>> components() { return components; }
    ...
    public List<CompletionSuggestion> getSuggestionsFor(final List<T> entries, final String currentToken, final int position, final TokenRange replacementRange) {
        ...
        if (!replacementRange.contains(position)) return List.of();
        final int suggestionIndex = replacementRange.rangeIndexAtPosition(position);
        return switch (suggestionIndex) {
            case 0 -> getSuggestionsForComponent(suggestionFrom, replacementRange.simplify());
            case 1 -> getSuggestionsForTerm(entries, replacementRange.range(0).substring(currentToken), "", replacementRange.simplify());
            case 2 -> getSuggestionsForTerm(entries, replacementRange.range(0).substring(currentToken), suggestionFrom, replacementRange.simplify());
            default -> List.of();
        };
    }
    ...
    public List<CompletionSuggestion> getSuggestionsForTerm(final List<T> entries, final String componentName, final String current, final TokenRange replacementRange) {
        ...
        final boolean startsWithQuote = !current.isEmpty() && "'\"`".indexOf(current.charAt(0)) >= 0;
    ...
    public static class Builder<T> {
        ...
        public Builder<T> defaultComponent(final String key, final SearchableComponent<T> component) {
            if (defaultComponent != null) throw new IllegalStateException("Cannot mark multiple components as a default component!");
            components.put(key, component);
