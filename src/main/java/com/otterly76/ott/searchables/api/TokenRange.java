...
public final class TokenRange implements Comparable<TokenRange>, Iterable<TokenRange> {
    ...
    public void addRange(final TokenRange range) { this.subRanges.add(range); }
    public void addRanges(final Collection<TokenRange> ranges) { this.subRanges.addAll(ranges); }
    public Set<TokenRange> subRanges() { return subRanges; }
    ...
    public int rangeIndexAtPosition(final int position) {
        if (!this.contains(position)) throw new IndexOutOfBoundsException();
        int i = 0;
        for (TokenRange subRange : subRanges()) {
            if (!subRange.contains(position)) { i++; continue; }
            break;
        }
        return i;
    }
    ...
    public boolean isEmpty() { return this.start() == this.end(); }
    public int length() { return this.end() - this.start(); }
    public int start() { return start; }
    public int end() { return end; }

    public TokenRange recalculate() {
        if (this.subRanges().isEmpty()) return this;
        int rangeStart = this.subRanges().stream().min(Comparator.comparing(TokenRange::end)).map(TokenRange::start).orElse(this.start());
        int rangeEnd = this.subRanges().stream().max(Comparator.comparing(TokenRange::end)).map(TokenRange::end).orElse(this.end());
        if (rangeStart == this.start() && rangeEnd == this.end()) return this;
    ...
    public String toString() {
        return "TokenRange{start=" + start + ", end=" + end + ", subRanges=" + subRanges + '}';
    }
