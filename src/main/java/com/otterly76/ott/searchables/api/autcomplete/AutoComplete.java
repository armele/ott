...
public class AutoComplete<T> extends AbstractWidget implements Consumer<String> {
    ...
    public boolean mouseScrolled(double xpos, double ypos, double xDelta, double yDelta) {
        if (isMouseOver(xpos, ypos) || this.editBox().isMouseOver(xpos, ypos)) {
            displayOffset = (int) Math.clamp(displayOffset - yDelta, 0, Math.max(this.suggestions.size() - maxSuggestions, 0));
    ...
    @Override
    public void renderWidget(@org.jetbrains.annotations.NotNull GuiGraphics guiGraphics, int mx, int my, float partial) {
        ...
        for (int i = displayOffset; i < Math.min(displayOffset + maxSuggestions, suggestions.size()); i++) {
            ...
            guiGraphics.drawString(Minecraft.getInstance().font, suggestion.display(), minX, minY + 1,
                    hovered ? Objects.requireNonNull(ChatFormatting.YELLOW.getColor()) : 0xFFFFFFFF);
        }
    ...
    public void scrollUp(int amount) { this.offsetDisplay(this.selectedIndex - amount); }
    public void scrollDown() { this.scrollDown(1); }
    public void scrollDown(int amount) { this.offsetDisplay(this.selectedIndex + amount); }

    private void offsetDisplay(int offset) {
        offset = Math.clamp(offset, 0, shownSuggestions() - 1);
        ...
        if (currentItem < this.displayOffset + halfSuggestions) {
            ...
            this.displayOffset = Math.clamp(currentItem - halfSuggestions, 0, Math.max(this.suggestions.size() - maxSuggestions, 0));
        }
    ...
    public int maxSuggestions() { return maxSuggestions; }
    public AutoCompletingEditBox<T> editBox() { return editBox; }

    @Override
    protected void updateWidgetNarration(@org.jetbrains.annotations.NotNull NarrationElementOutput output) {}
}
