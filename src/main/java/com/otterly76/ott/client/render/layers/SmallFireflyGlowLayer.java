...
@OnlyIn(Dist.CLIENT)
public class SmallFireflyGlowLayer extends GeoRenderLayer<SmallFirefly> {
    ...
    public SmallFireflyGlowLayer(GeoRenderer<SmallFirefly> entityRendererIn) {
        super(entityRendererIn);
    }

    @Override
    public void render(PoseStack poseStack, SmallFirefly entity, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTicks, int packedLightIn, int packedOverlay) {
        if (!entity.isGlowing()) {
            return;
        }
