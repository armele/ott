...
@OnlyIn(Dist.CLIENT)
public class ButterflyGlowLayer extends GeoRenderLayer<Butterfly> {
    public ButterflyGlowLayer(GeoRenderer<Butterfly> entityRendererIn) {
        super(entityRendererIn);
    }

    @Override
    public void render(PoseStack poseStack, Butterfly entity, BakedGeoModel bakedModel, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, float partialTicks, int packedLightIn, int packedOverlay) {
        ResourceLocation glowTexture = getGlowTexture(entity);
        if (glowTexture == null) {
            return;
        }
    ...
    private ResourceLocation getGlowTexture(Butterfly entity) {
